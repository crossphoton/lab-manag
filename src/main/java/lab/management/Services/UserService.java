...
public class UserService {
    ...
    public static String signin(HashMap<Object, Object> user) throws NotFound, ServerError, NotAllowed {
        ...
        try{
            List<QueryDocumentSnapshot> query = userRef.whereEqualTo("username", username)
            ...
            if(query.size() != 1){
                ...
                String foundPassword = query.get(0).getReference()
                            .get().get().getData().get("password").toString();

                String role = query.get(0).getReference()
                            .get().get().getData().get("role").toString();

                if(foundPassword.equals(user.get("password"))){

                    query.get(0).getReference()
                            .update("lastLogin", new SimpleDateFormat("yyyy-mm-dd hh:mm:ss")
    ...
    public static String signup(Users user) throws ServerError, NotAllowed {
        ...
        try{
            List<QueryDocumentSnapshot> alreadyExist = userRef.whereEqualTo("username", user.username)
                        .get().get().getDocuments();
